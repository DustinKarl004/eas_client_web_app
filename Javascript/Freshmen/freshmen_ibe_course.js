...
async function fetchCourseResults(searchTerm = '') {
    try {
        ...
        if (!selectedCourse || (selectedCourse !== 'BSBA-HRM' && selectedCourse !== 'BSEntrep')) {
        ...
        const resultsQuery = query(collection(db, 'freshmen_examinees_result'), where('course', '==', selectedCourse));
